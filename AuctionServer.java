...
public class AuctionServer implements Auction {
    ...
    private static void generateKeyPair() throws Exception{
        ...
        public int register(String email) throws RemoteException{
            ...
            usersMap.put(userID, email);    //place into hashmap, userID being the key for each email, incase we need a user's email
            return userID;
        }
        ...
        public int newAuction(int userID, AuctionSaleItem item) throws RemoteException{
            ...
            userAuctionsMap.put(userID, item);   //place into hashmap, userID being the key for each AuctionSaleItem, to know who created the auction
            
            //create new AuctionItem from AuctionSaleItem info
    
            AuctionItem newItem = createItem(itemID, item.name, item.description, 0);
            itemsMap.put(itemID, newItem); //place into hashmap, the itemID being the key for the AuctionItem, so we have all items saved in one place 
        ...
        public AuctionResult closeAuction(int userID, int itemID){
            ...
            if (userAuctionsMap.containsKey(userID) && item.itemID == itemID) { //Check the user closing the auction is the one who created it
                ...
                if(item.highestBid >= saleItem.reservePrice){   //Check if the reserve price has been met
                    ...
                    AuctionResult result = new AuctionResult(); //Create a new instance of an AuctionResult using details from various hashmaps
        ...
        public boolean bid(int userID, int itemID, int price) throws RemoteException{
                ...
                if(price > item.highestBid){    //Check that the amount offered is higher than the current bid
                    ...
                    highestBidderMap.put(item.itemID, userID);   //place in hashmap, showing who the highest bidder (userID) is for each given itemID, this overwrites the previous highest bidder
                    return true;
                }
                else{
